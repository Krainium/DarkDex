...
int main(int argc,char**argv){
 ...
 char mpath[256];snprintf(mpath,256,"/proc/%d/maps",pid);FILE*mf=fopen(mpath,"r");if(!mf)return 3;
 ...
 while(fgets(line,sizeof line,mf)){
  ...
  if(perms[0]!='r')continue;uint64_t sz=e-s;if(sz==0||sz>900ULL*1024*1024)continue;
  ...
  for(size_t i=0;i+0x70<=(size_t)n;i+=4){
   ...
   char op[640];snprintf(op,640,"%s/carve_%02d_%s.dex",out,dex,cd?"cdex":"dex");FILE*of=fopen(op,"wb");
  ...
  for(size_t i=4;i+64<(size_t)n;i+=4){
   ...
   if(!moff||!cds||!sids)continue;uint64_t mla=s+(i-4),dst=mla-moff;uint32_t fsz=moff+4+cnt*12;
   if(dst<s||dst+fsz>e||fsz<0x70||fsz>96u*1024*1024)continue;
 ...
 printf("DARKDEX_DONE dex=%d recovered=%d urls=%zu classes=%zu packer=%zu\n",dex,rec,urls.size(),classes.size(),packer.size());
 return 0;
}
